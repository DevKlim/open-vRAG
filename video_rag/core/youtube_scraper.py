...
def get_most_replayed_timestamps(youtube_url, video_duration, logger):
    ...
    try:
        ...
        driver.quit()
        logger.info("ChromeDriver closed. Parsing SVG path data.")
        
        points = re.findall(r'C\s*[\d\.]+,([\d\.]+)\s*[\d\.]+,([\d\.]+)\s*[\d\.]+,(.+?)\s', d_attribute)
        y_coords = [float(p[2]) for p in points]
        
        if not y_coords:
            logger.warning("Failed to parse heat map coordinates from SVG path.")
            st.warning("Failed to parse heat map data from the page source.")
            return []
        ...
        if len(peak_indices) == 0:
            logger.info("No significant replay peaks found based on threshold analysis.")
            st.write("No significant replay peaks found based on the threshold.")
            return []
        ...
        for idx in peak_indices:
            if idx > last_index + 5:
                significant_timestamps.append(timestamps[idx])
            last_index = idx
